...
export default function ProductsManagerPage() {
  ...
  const filteredProducts = useMemo(() => {
    return products.filter((p) => {
      ...
      const matchesStock =
        stockFilter === "all" || p.availability === stockFilter;
  ...
  const handleImportCSV = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (event) => {
      ...
      if (newProducts.length > 0) {
        ...
        alert(
          "Failed to parse CSV. Make sure headers match ID,Name,Category,Price,Availability,Warranty."
        );
      }
    };
  ...
  const isRoleRestricted =
    currentRole === "Sales Staff" || currentRole === "Support Staff";
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-zinc-500 text-sm">
            Manage store inventory, edit configurations, run CSV imports, and
            trigger bulk jobs.
          </p>
        </div>
            ...
            <tbody className="divide-y divide-black/5 dark:divide-white/5 font-semibold text-brand-ink dark:text-white">
              {paginatedProducts.length > 0 ? (
                      ...
                      <td className="px-6 py-4">
                        <div className="flex flex-wrap gap-1">
                          {p.condition.map((c) => (
                            <span
                              key={c}
                              className="text-[9px] px-1.5 py-0.5 bg-zinc-100 dark:bg-zinc-800 text-zinc-600 dark:text-zinc-400 rounded-md"
                            >
                              {c}
                            </span>
                          ))}
                          ...
                          <button
                            onClick={() => {
                              if (!isRoleRestricted) duplicateProduct(p.id);
                                ...
                                alert(
                                  "Access Denied: Restricted role permissions."
                                );
                            }}
                            title="Duplicate SKU"
                            className="p-1.5 bg-zinc-50 hover:bg-zinc-100 text-zinc-600 rounded-lg dark:bg-zinc-800 dark:text-zinc-300 dark:hover:bg-zinc-700 transition"
                          ...
                          <button
                            onClick={() => {
                              if (currentRole !== "Support Staff") {
                                ...
                                alert(
                                  "Access Denied: Support Staff do not have deletion permissions."
                                );
                              }
                            }}
                            title="Delete"
                ...
                <tr>
                  <td
                    colSpan={7}
                    className="px-6 py-12 text-center text-zinc-400 font-semibold"
                  >
