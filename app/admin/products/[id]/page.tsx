...
export default function EditProductPage({ params }: Props) {
  ...
  useEffect(() => {
    if (originalProduct) {
      ...
      setSpecs(
        parsedSpecs.length ? parsedSpecs : [{ key: "Display", value: "" }]
      );
    ...
    return (
      <div className="p-12 text-center max-w-md mx-auto space-y-4">
        ...
        <p className="text-sm text-zinc-500">
          The product SKU code may have been deleted or archived.
        </p>
        <Link
          href="/admin/products"
          className="inline-block px-4 py-2 bg-brand-blue text-white rounded-full text-xs font-bold shadow"
        >
          Back to Catalog
        </Link>
      </div>
  ...
  const triggerImageUpload = () => {
    setUploadedImages((prev) => [
      ...prev,
      `/iphone-detail-mock-${prev.length + 1}.png`
    ]);
  };
  ...
  const handleSave = (e: React.FormEvent) => {
    ...
    if (currentRole === "Sales Staff" || currentRole === "Support Staff") {
      alert(
        `Access Denied: Restricted role (${currentRole}) cannot modify catalog products.`
      );
      return;
    }
        ...
        <div className="space-y-6">
          ...
          <div className="p-6 bg-white dark:bg-zinc-900 rounded-[2rem] border border-black/5 dark:border-white/5 shadow-sm space-y-4">
            ...
            <p className="text-xs text-zinc-400">
              Reorder and prioritize product images
            </p>
            ...
            <div className="space-y-3">
              {uploadedImages.map((img, i) => (
                <div
                  key={i}
                  className="flex items-center gap-3 p-2 bg-zinc-50 dark:bg-zinc-800 rounded-xl border border-black/5 dark:border-white/5 justify-between"
                >
