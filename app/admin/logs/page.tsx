...
export default function SecurityLogsPage() {
  ...
  const handleClearLogs = () => {
    if (currentRole !== "Owner") {
      alert(
        "Access Denied: Only Owner accounts can clear security audit trails."
      );
      return;
    }
    if (
      window.confirm(
        "Are you sure you want to purge all security logs? This action is irreversible."
      )
    ) {
      clearAuditLogs();
    }
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-zinc-500 text-sm">
            Read-only tamper-evident logs tracing admin edits, configuration
            updates, and role hotswaps.
          </p>
        </div>
        <button
      ...
      <div className="bg-white dark:bg-zinc-900 rounded-[2rem] border border-black/5 dark:border-white/5 shadow-sm overflow-hidden">
        <div className="p-4 border-b border-black/5 dark:border-white/5 bg-zinc-50/50 dark:bg-zinc-850/20 flex items-center justify-between">
          <h3 className="font-bold text-brand-ink dark:text-white text-xs flex items-center gap-1.5">
            <LockKeyhole className="h-4 w-4 text-brand-blue" /> Tamper-Proof
            Audit Log Feed
          </h3>
        ...
        <div className="divide-y divide-black/5 dark:divide-white/5 max-h-[550px] overflow-y-auto">
          {filteredLogs.length > 0 ? (
            filteredLogs.map((log) => (
              <div
                key={log.id}
                className="p-5 flex flex-col sm:flex-row sm:items-start gap-4 transition hover:bg-zinc-50 dark:hover:bg-zinc-850/20"
              >
                {/* Icon indicator */}
                ...
                <div className="flex-1 space-y-1">
                  <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-1">
                    <p className="text-sm font-extrabold text-brand-ink dark:text-white">
                      {log.action}
                    </p>
                  ...
                  <div className="flex flex-wrap gap-2 pt-1.5 text-[10px] font-semibold text-zinc-400">
                    ...
                    <span>
                      Role Group:{" "}
                      <strong className="text-brand-blue">{log.role}</strong>
                    </span>
                    {log.entityId && (
                      <>
                        <span>&bull;</span>
                        <span>
                          Entity ID:{" "}
                          <strong className="text-zinc-600 dark:text-zinc-400 font-mono">
                            {log.entityId}
                          </strong>
                        </span>
                      </>
