...
export function CheckoutForm() {
  ...
  const [submittedRequestPriced, setSubmittedRequestPriced] = useState(false);
  ...
  const shippingFee =
    hasRequestPricedItems || fulfillment === "pickup"
      ? 0
      : subtotal > 1000
        ? 0
        : 50;
  ...
  async function onSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    setSubmittedRequestPriced(hasRequestPricedItems);
    clearCart();
    setSubmitted(true);
  }
    ...
    return (
      <div className="max-w-2xl mx-auto rounded-[2.5rem] bg-white dark:bg-zinc-900 border border-emerald-500/30 p-8 sm:p-12 text-center shadow-2xl space-y-6">
        ...
        <p className="text-xs text-zinc-500 dark:text-zinc-400 max-w-md mx-auto leading-relaxed">
          Your reference is{" "}
          <strong className="text-brand-blue font-mono">{orderId}</strong>.
          TroyX iStore will confirm availability, final pricing, and fulfillment
  ...
  return (
    <div className="grid gap-8 lg:grid-cols-[1fr_380px]">
      ...
      <form
        onSubmit={onSubmit}
        className="space-y-6 rounded-[2.5rem] border border-black/5 bg-white p-6 sm:p-8 shadow-sm dark:border-white/10 dark:bg-zinc-900"
      >
        {/* Fulfillment Selection */}
        <div>
          <label className="block text-xs font-bold uppercase text-zinc-400 tracking-wider mb-3">
            1. Select Delivery or Store Pickup
          </label>
            ...
            <div className="grid gap-4 sm:grid-cols-3">
              <div className="sm:col-span-2">
                ...
                <Input
                  name="address"
                  required
                  placeholder="House number / Landmark address"
                  className="h-11 text-xs"
                />
              </div>
              <div>
        ...
        <div className="space-y-4 pt-4 border-t border-black/5 dark:border-white/10">
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            {[
              {
                id: "paystack",
                name: "Paystack (Mobile Money / Cards)",
                icon: CreditCard
              },
              {
                id: "mtn_momo",
            ...
            >
              <div>
                ...
                <p className="text-[10px] text-zinc-400">
                  Qty: {item.quantity} &bull; {item.color || "Standard"}
                </p>
              </div>
