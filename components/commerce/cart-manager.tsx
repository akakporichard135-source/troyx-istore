...
export function CartManager() {
  ...
  const hasRequestPricedItems = enriched.some(
    (item) => (item.product?.price || 0) <= 0
  );
  const subtotal = enriched.reduce(
    (sum, item) => sum + (item.product?.price || 0) * item.quantity,
    0
  );
  const vatAmount = hasRequestPricedItems ? 0 : Math.round(subtotal * 0.15);
  const shippingFee = hasRequestPricedItems ? 0 : subtotal > 1000 ? 0 : 50;
  const grandTotal = Math.max(
    0,
    subtotal + vatAmount + shippingFee - appliedDiscount
  );
  ...
  const handleApplyCoupon = () => {
    ...
    const found = coupons.find(
      (c) =>
        c.code.toUpperCase() === couponCode.trim().toUpperCase() && c.active
    );
  ...
  if (!enriched.length) {
    return (
      <EmptyState
        ...
        description="Add a device or accessory to start checkout."
      />
    );
  }
    ...
    <div className="space-y-12">
      <div className="grid gap-8 lg:grid-cols-[1fr_380px]">
          ...
          {enriched.map((item) => (
            ...
            >
              <Link
                ...
                className="relative aspect-square overflow-hidden rounded-2xl bg-brand-mist dark:bg-zinc-950"
              >
                {item.product && (
                  <Image
              ...
              <div className="flex items-center gap-3 sm:flex-col sm:items-end sm:justify-between">
                ...
                <div className="flex items-center rounded-full border border-black/10 bg-zinc-50 dark:border-white/10 dark:bg-zinc-800">
                  <button
                    ...
                    className="h-9 w-9 flex items-center justify-center text-zinc-600 dark:text-zinc-300"
                    onClick={() =>
                  ...
                  <span className="w-8 text-center text-xs font-bold text-brand-ink dark:text-white">
                    {item.quantity}
                  </span>
                  <button
                    type="button"
        ...
        <aside className="h-fit rounded-[2.5rem] border border-black/5 bg-white p-6 shadow-sm dark:border-white/10 dark:bg-zinc-900 space-y-5">
          ...
          <div className="space-y-2 pt-2">
            <div className="flex gap-2">
              ...
              <Button
                ...
                className="h-10 px-4 text-xs font-bold bg-brand-blue text-white"
              >
                Apply
              </Button>
            </div>
            ...
            {couponMsg && (
              <p
                className={`text-[11px] font-semibold ${appliedDiscount > 0 ? "text-emerald-500" : "text-red-400"}`}
              >
                {couponMsg}
              </p>
            )}
          ...
          <LinkButton
            href="/checkout"
            className="w-full h-11 text-xs font-bold bg-brand-blue hover:bg-brand-blue/90 text-white"
          >
            Proceed to Checkout
